...
public:
	...
	PlayerController& GetController(int no);

	/// @brief コントローラー生成
	/// @param no プレイヤー番号
	/// @param key 任意のキー
	std::weak_ptr<PlayerController> CreateController(int no, int key);
...
private:
	...
	// インプットコントローラー
	std::shared_ptr<PlayerController> controller_[MAX_PLAYER];
